...
void produce_messages(libkafka_asio::Connection &connection, const std::string& topic, int count, std::promise<int> * promObj) {
  if(count<1) {
    promObj->set_value(0);
    return;
  }
  ...
  for(int i=0; i<count; i++) {
    std::stringstream ss;
    ss << "message from " << __func__ << " in thread " << gettid() << " at " <<get_current_datetime_str() ;
    ...
    { // it is running in io_service thread
      if (err)
      {
      ...
      std::cout <<"tid " << gettid()<< " Successfully produced message! " << message_count.load() << std::endl;
  ...
  std::cout << __func__ << " exit from tid " << gettid() << " message_count is " << message_count.load() << std::endl;
}
...
int main(int argc, char **argv)
{
  ...
  desc.add_options()("help,h", "produce help message")
           ("port", boost::program_options::value<decltype(port)>( &port)->default_value(port), "Port number to connect")
    ( "server", boost::program_options::value<decltype(server_address)>(&server_address)->default_value(server_address),
      "Server address")
	  ( "produce_count", boost::program_options::value<int>(&produce_count)->default_value(produce_count),
      "Produce specified number of messages, default is 0")
      ( "producer_timeout", boost::program_options::value<int>(&producer_timeout_seconds)->default_value(producer_timeout_seconds),
      "Producer timeout in 5 seconds by default")
      ( "consumer_idle_exit", boost::program_options::value<int>(&consumer_idle_exit)->default_value(consumer_idle_exit),
  ...
  std::thread run_thread([&]{ ios.run(); std::cout<<"boost::asio thread " << gettid() << " exit\n"; });
  std::string broker_address = server_address + ":" + std::to_string(port); // "localhost:9092"
  ...
  // try: bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic test --from-beginning
  ...
  std::thread producer_thread(produce_messages, std::ref(connection), std::ref(topic), produce_count, &promiseObj);
  ...
  std::thread consumer_thread([&] {
    ...
    { // this code is running in ios_service thread
      if (err)
      {
        std::cerr
